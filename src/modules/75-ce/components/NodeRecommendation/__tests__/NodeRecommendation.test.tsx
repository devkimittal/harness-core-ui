...
describe('test cases for node recommendations details', () => {
  ...
  test('should be able to show error while fetching recommendation', async () => {
    const { getByTestId, queryByText } = render(
      <TestWrapper>
        <NodeRecommendationDetails
          recommendationDetails={MockRecommendationData.data.recommendationDetails as NodeRecommendationDto}
    ...
    act(() => {
      fireEvent.change(getByTestId('minnodes-input'), { target: { value: 0 } })
    })
    ...
    await waitFor(() => {
      expect(queryByText('ce.nodeRecommendation.fetchRecommendationError')).toBeDefined()
    })
  })
